...
module.exports.toP=function(val, def = {value:1920}) {
    if(!val || val.value === 0) return '0';
    return (val.value / def.value) * 100 + "%";
}

module.exports.toW=function(val, def = {value:1920}) {
    if(!val || val.value === 0) return '0';
    return (val.value / def.value) * 100 + "vw";
}

module.exports.toH=function(val, def = {value:1920}) {
    if(!val || val.value === 0) return '0';
    return (val.value / def.value) * 100 + "vh";
}

module.exports.toMi=function(val, def = {value:1920}) {
    if(!val || val.value === 0) return '0';
    return (val.value / def.value) * 100 + "vmin";
}

module.exports.toM=function(val, def = {value:1920}) {
    if(!val || val.value === 0) return '0';
